...
public class FunCommandsModule : IModule
{
    ...
    public async Task Poll(CommandContext ctx, TimeSpan duration, params DiscordEmoji[:+1:, :-1:] emojiOptions) //not sure if i can use :+1:, :-1: here, but i guess this will be default values if not defined
    {
        var intr = ctx.Client.GetInteractivityModule();
        ...
        var pollEmbed = new DiscordpollEmbedBuilder
        {
            Title = "The poll is about: " + response,
            Description = "React with:" + string.Join(" ", pollEmojiOptions) + " to show others what you think.\nYou've got " + duration + " go go go!"
        };

        var pollMsg = await ctx.RespondAsync(embed: pollEmbed);

        for (var i = 0; i < emojiOptions.Length; i++)
            await pollMsg.CreateReactionAsync(emojiOptions[i]);

        var result = await intr.CollectReactionsAsync(pollMsg, duration);
        var results = result.Reactions.Where(x => emojiOptions.Contains(x.Key))
            .Select(x => $"{x.Key}: {x.Value}");

        await ctx.RespondAsync("Poll results:" + string.Join("\n", results));
//test
        }        
