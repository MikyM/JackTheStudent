            ...
            if (uniClass == "." && classType == "." && group == ".") {              
                ...
                if (teamsLinks.Count == 0) {
                    ...
                    foreach (TeamsLink teamsLink in teamsLinks) {
                        ...
                        result = $"{result} \n{CultureInfo.CurrentCulture.TextInfo.ToTitleCase(teamsLink.Class)} {teamsLink.ClassType} teams link for {chosenGroupString}, takes place on {teamsLink.Date.ToString().Trim()}. Link: {teamsLink.ShortenedLink}{(teamsLink.AdditionalInfo.Equals("") ? "" : $"Additional info: {teamsLink.AdditionalInfo}.")}";                       
                    }
                }
